...
// 초기 프롬프트 (기존 데이터 없을 때만)
$stmt = $pdo->query("SELECT COUNT(*) FROM `prompts`");
if ((int) $stmt->fetchColumn() === 0) {
    $stmt = $pdo->prepare("INSERT INTO `prompts` (`type`, `content`, `is_active`) VALUES (?, ?, 1)");
    $stmt->execute(['weekly', '다음 사용자들의 이름을 기반으로 각각 1부터 45 사이의 중복 없는 행운의 로또 번호 6개를 생성해주세요. 반드시 JSON 배열로 응답하세요. 사용자 목록: {names}']);
    $stmt->execute(['fixed', '다음 사용자의 이름에서 느껴지는 기운, 획수, 의미를 종합적으로 분석하여 이 이름만의 고유한 운명의 번호 6개(1~45, 중복 없음)를 생성해주세요. 이 번호는 이 이름에 평생 부여되는 고유번호입니다. 반드시 JSON 배열로 응답하세요. 사용자 목록: {names}']);
}
